...
def solve_with_bitmasks_new(board):
    N = board.size
    ALL_BITS = (1 << N) - 1  # Mask with all bits set for numbers 1 to N
    rows = [0] * N
    cols = [0] * N
    boxes = [0] * N

    box_lookup = [(i // int(np.sqrt(N))) * int(np.sqrt(N)) + (j // int(np.sqrt(N)))
                  for i in range(N) for j in range(N)]
    empty_cells = []
    for i in range(N):
        for j in range(N):
            if board.grid[i][j] != '.':
                val = int(board.grid[i][j])
                bit = 1 << (val - 1)
                rows[i] |= bit
                cols[j] |= bit
                boxes[box_lookup[i * N + j]] |= bit
            else:
    ...
    def solve(pos=0):
        ...
        i, j = empty_cells[pos]
        box_idx = box_lookup[i * N + j]

        # Get all possible values using bitmask
        used = rows[i] | cols[j] | boxes[box_idx]
        available = ALL_BITS & ~used

        while available:
            # Get least significant bit
            bit = available & -available
            # Convert bit to number (1-based)
            num = (bit & -bit).bit_length()
            board.grid[i][j] = num
            rows[i] |= bit
            cols[j] |= bit
            boxes[box_idx] |= bit
...
def solve_with_crosshatching_new(board):
    N = board.size
    box_size = int(np.sqrt(N))

    # Position tracking
    pos = {}  # tracks positions of each number
    rem = {}  # tracks remaining count for each number
    graph = {}  # tracks possible positions for each number

    # Pre-calculate box indices
    box_lookup = [(i // box_size) * box_size + (j // box_size)
                  for i in range(N) for j in range(N)]

    # Use arrays for efficient checking
    rows = [[False] * (N + 1) for _ in range(N)]
    cols = [[False] * (N + 1) for _ in range(N)]
    boxes = [[False] * (N + 1) for _ in range(N)]

    # Initialize tracking and arrays
    for i in range(N):
        for j in range(N):
            if board.grid[i][j] != '.':
                num = int(board.grid[i][j])
                ...
                # Update checking arrays
                rows[i][num] = True
                cols[j][num] = True
                boxes[box_lookup[i * N + j]][num] = True
    ...
    # Sort by remaining count for better pruning
    rem = dict(sorted(rem.items(), key=lambda x: x[1]))
    numbers = list(rem.keys())

    def is_safe(row, col, num):
        box_idx = box_lookup[row * N + col]
        return not (rows[row][num] or cols[col][num] or boxes[box_idx][num])
    ...
    def build_graph():
        for num in range(1, N + 1):
            ...
            possible_cols = set(range(N))

            # Remove occupied positions
            for r, c in pos[num]:
                possible_rows.discard(r)
                possible_cols.discard(c)
                ...
                for c in possible_cols:
                    if board.grid[r][c] == '.':
                        if is_safe(r, c, num):
                            valid_cols.append(c)
                if valid_cols:
                    graph[num][r] = valid_cols

    def place_number(row, col, num):
        board.grid[row][col] = num
        rows[row][num] = True
        cols[col][num] = True
        boxes[box_lookup[row * N + col]][num] = True

    def remove_number(row, col, num):
        board.grid[row][col] = '.'
        rows[row][num] = False
        cols[col][num] = False
        boxes[box_lookup[row * N + col]][num] = False

    def solve_recursive(num_idx=0, row_idx=0):
        if num_idx >= len(numbers):
            return True

        num = numbers[num_idx]
        rows_list = list(graph[num].keys())

        if row_idx >= len(rows_list):
            return solve_recursive(num_idx + 1, 0)

        row = rows_list[row_idx]
        for col in graph[num][row]:
            if board.grid[row][col] == '.':
                if is_safe(row, col, num):
                    place_number(row, col, num)
                    if solve_recursive(num_idx, row_idx + 1):
                        return True
                    remove_number(row, col, num)

        return False

    # Build initial graph and solve
    build_graph()
...
def solve_merge(board):
    ...
    def findvnum(_y, _x):
        ...
        for i in range(size):
            if board.grid[_y][i] != '.' and board.grid[_y][i] in vnum:
                vnum.remove(board.grid[_y][i])

        for i in range(size):
            if board.grid[i][_x] != '.' and board.grid[i][_x] in vnum:
                vnum.remove(board.grid[i][_x])
        ...
        for i in range(gy * v1, gy * v1 + v1):
            for j in range(gx * v1, gx * v1 + v1):
                if board.grid[i][j] != '.' and board.grid[i][j] in vnum:
                    vnum.remove(board.grid[i][j])

        return vnum

    def get_setting():
        vnum_grid = [[[] for _ in range(board.size)] for _ in range(board.size)]
        num_vnum = np.array([[int(99) for _ in range(len(board.grid[0]))] for _ in range(len(board.grid[0]))])
    ...
    def update_vgrid(vnum_grid, num_vnum, _y, _x, num):  # i in for loop
        deleted_position = []
        for i in range(board.size):
            if board.grid[i][_x] == '.' and (num in vnum_grid[i][_x]):
                vnum_grid[i][_x].remove(num)
                num_vnum[i][_x] -= 1
                deleted_position.append([i, _x])

        for j in range(board.size):
            if board.grid[_y][j] == '.' and (num in vnum_grid[_y][j]):
                vnum_grid[_y][j].remove(num)
                num_vnum[_y][j] -= 1
                deleted_position.append([_y, j])

        v1 = int(np.sqrt(board.size))
        _x1 = _x // v1
        _y1 = _y // v1

        for i in range(_y1 * v1, _y1 * v1 + v1):
            for j in range(_x1 * v1, _x1 * v1 + v1):
                if board.grid[i][j] == '.' and (num in vnum_grid[i][j]):
                    vnum_grid[i][j].remove(num)
                    num_vnum[i][j] -= 1
                    deleted_position.append([i, j])
    ...
    def reset_vgrid(vnum_grid, num_vnum, i, dp):
        for _y, _x in dp:
            vnum_grid[_y][_x].append(i)
            num_vnum[_y][_x] += 1

    def ch_backtracking(vnum_grid, num_vnum, board, filled_cell):
        if filled_cell == int(board.size**2*0.4):
            return solve_with_crosshatching_new(board)
        least_num = num_vnum.min()
        _y, _x = np.argwhere(num_vnum == least_num)[0]
        vnum = vnum_grid[_y][_x]
        num_vnum[_y][_x] = 99
        ...
        for i in vnum:
            board.grid[_y][_x] = i
            dp = update_vgrid(vnum_grid, num_vnum, _y, _x, i)
            if ch_backtracking(vnum_grid, num_vnum, board, filled_cell):
                return True
            reset_vgrid(vnum_grid, num_vnum, i, dp)
            board.grid[_y][_x] = '.'
        num_vnum[_y][_x] = least_num
        return False
    ...
    vnum_grid, num_vnum = get_setting()
    filled_cell = 0
    return ch_backtracking(vnum_grid, num_vnum, board, filled_cell)
